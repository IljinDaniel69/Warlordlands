...
app.use(
  session({
    ...
    secret:
      ...
      "admin-secret-key-different-from-game",
    resave: false,
...
app.get("/admin/game_turns/edit/:id", requireAuth, async (req, res) => {
  ...
  try {
    const [rows] = await pool.execute("SELECT * FROM game_turns WHERE id = ?", [
      id,
    ]);
...
app.get("/admin/game_turns/add", requireAuth, async (req, res) => {
  try {
    ...
    const [maxTurnResult] = await pool.execute(
      "SELECT MAX(turn_number) as maxTurn FROM game_turns"
    );
    ...
    const defaultCommandDeadline = new Date(
  ...
  try {
    ...
    try {
      ...
      if (currentTurnResult.length > 0) {
        ...
        console.log(
          `Processing commands for turn ${currentTurn.turn_number} before creating new turn...`
        );
        ...
        commandResults = await gameTurnManager.processPendingCommands(
          currentTurn.id
        );
...
app.get("/admin/:table/edit/:id", requireAuth, async (req, res) => {
  ...
  try {
    const [rows] = await pool.execute(`SELECT * FROM ${table} WHERE id = ?`, [
      id,
    ]);
